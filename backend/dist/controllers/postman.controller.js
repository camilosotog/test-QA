...
exports.getPostmanResults = getPostmanResults;
exports.getPostmanResultsWithAssertions = getPostmanResultsWithAssertions;
exports.deletePostmanResults = deletePostmanResults;
exports.getContractTestResults = getContractTestResults;
exports.getResponseTestResults = getResponseTestResults;
exports.getControlledResponseTestResults = getControlledResponseTestResults;
exports.runPostmanFromUrl = runPostmanFromUrl;
exports.runPostmanWithRetry = runPostmanWithRetry;
exports.testLongRequest = testLongRequest;
exports.testNewman = testNewman;
exports.processExistingResults = processExistingResults;
...
async function runPostmanCollection(req, res) {
    try {
        ...
        const { collectionPath, collectionUrl, collectionId, environmentPath, environmentUrl, environmentId, collectionName, environmentName, postmanApiKey } = req.body;
        ...
        const isFilePath = (str) => !isUrl(str) && !isId(str);
...
async function saveResultsToDatabase(data, collectionName, environmentName) {
    const savedResults = [];
    try {
        ...
        server_1.io.emit('postman:progress', initialProgress);
        ...
        for (let i = 0; i < executions.length; i++) {
            ...
            server_1.io.emit('postman:progress', progressData);
        ...
        `;
              ...
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
                        const [result] = await db_1.pool.execute(truncateQuery, [
        ...
        server_1.io.emit('postman:progress', completedProgress);
        return savedResults;
    }
    catch (error) {
...
async function saveAssertions(postmanResultId, assertions) {
    for (let j = 0; j < assertions.length; j++) {
      ...
      `;
            ...
            await db_1.pool.execute(assertionQuery, truncatedData);
        }
        catch (assertionError) {
...
async function getPostmanResults(req, res) {
    try {
        await ensurePostmanTable();
        const { collection_name, environment_name, status, limit = 100, offset = 0 } = req.query;
...
async function getPostmanResultsWithAssertions(req, res) {
    try {
        await ensurePostmanTable();
        const { collection_name, environment_name, status, limit = 100, offset = 0 } = req.query;
        // Convertir limit y offset a números enteros válidos
    ...
    `;
        ...
        const [results] = await db_1.pool.execute(query, params);
        // Para cada resultado, obtener las assertions individuales
        const resultsWithAssertions = await Promise.all(results.map(async (result) => {
            const assertionsQuery = `
          SELECT * FROM postman_assertions 
          WHERE postman_result_id = ? 
          ORDER BY id
        `;
            const [assertions] = await db_1.pool.execute(assertionsQuery, [result.id]);
            return {
                ...result,
...
async function deletePostmanResults(req, res) {
    try {
        const { ids } = req.body;
        ...
        const [result] = await db_1.pool.execute(query, ids);
        res.json({
...
async function getContractTestResults(req, res) {
    try {
        await ensurePostmanTable();
...
async function getResponseTestResults(req, res) {
    try {
        await ensurePostmanTable();
        const { projectName } = req.body;
        if (!projectName) {
...
async function getControlledResponseTestResults(req, res) {
    try {
        await ensurePostmanTable();
        const { projectName } = req.body;
        if (!projectName) {
...
async function runPostmanFromUrl(req, res) {
    try {
        // Configurar timeout más largo para la respuesta HTTP
        req.setTimeout(600000); // 10 minutos
        res.setTimeout(600000); // 10 minutos
        await ensurePostmanTable();
        const { collectionUrl, collectionId, environmentUrl, environmentId, collectionName, environmentName, postmanApiKey } = req.body;
...
async function runPostmanWithRetry(req, res) {
    try {
        await ensurePostmanTable();
        const { projectName, maxRetries = 2 } = req.body;
        if (!projectName) {
        ...
        server_1.io.emit('postman:progress', startProgress);
...
async function executeNewmanInBackground(newmanCommand, resultsPath, collectionName, environmentName, maxRetries = 2) {
    let attempt = 0;
    let lastError;
...
async function testLongRequest(req, res) {
    try {
...
async function testNewman(req, res) {
    try {
...
async function processExistingResults(req, res) {
    try {
        await ensurePostmanTable();
...
//# sourceMappingURL=postman.controller.js.map