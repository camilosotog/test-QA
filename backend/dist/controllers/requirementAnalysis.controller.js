...
exports.analyzeAndPublish = exports.commentOnJira = exports.publishAnalysisToJira = exports.analyzeRequirementWithFiles = exports.analyzeRequirement = void 0;
const requirement_analyzer_mcp_1 = __importDefault(require("../mcp/requirement-analyzer-mcp"));
const axios_1 = __importDefault(require("axios"));
// eslint-disable-next-line @typescript-eslint/no-require-imports
const pdfParse = require("pdf-parse");
const mammoth_1 = __importDefault(require("mammoth"));
const tesseract_js_1 = __importDefault(require("tesseract.js"));
const analyzer = new requirement_analyzer_mcp_1.default();
/**
...
const extractTextFromFile = async (file) => {
    ...
    if (mimetype === "application/pdf") {
        ...
        if (data.text.trim().length < 20) {
            ...
            const { data: ocrData } = await tesseract_js_1.default.recognize(buffer, "spa+eng", {
 ...
 */
const fetchJiraIssue = async (jiraUrl) => {
    try {
        ...
        const response = await axios_1.default.get(jiraApiUrl, {
            auth: {
                username: jiraEmail,
                password: jiraToken,
            },
        ...
        if (code === "ENOTFOUND" || code === "ECONNREFUSED" || code === "ETIMEDOUT") {
            ...
            console.error(`[Jira] Error al obtener issue (${status || code || "desconocido"}):`, error.message);
        }
        throw error;
    }
};
    ...
    try {
        ...
        if (jiraUrl) {
            try {
                ...
                if (!customText) {
                    const code = jiraError?.code;
                    const httpStatus = jiraError?.response?.status;
        ...
        res.status(500).json({
            success: false,
            error: error instanceof Error ? error.message : "Error al analizar requerimiento",
        });
    }
};
 ...
 */
const analyzeRequirementWithFiles = async (req, res) => {
    try {
...
exports.analyzeRequirementWithFiles = analyzeRequirementWithFiles;
/**
 * Genera PDF y lo sube como comentario en Jira
 * POST /api/requirement-analysis/publish-to-jira
 */
const publishAnalysisToJira = async (req, res) => {
    try {
        ...
        if (!jiraToken || !jiraEmail) {
            console.warn("[Publish] Credenciales de Jira no disponibles - modo simulado");
            return res.json({
                success: true,
                data: {
                    issueKey: issueKey,
        ...
        try {
            ...
            const attachmentResponse = await axios_1.default.post(attachmentUrl, formData, {
                auth: {
                    username: jiraEmail,
                    password: jiraToken,
                },
            ...
            const errorStatus = jiraError?.response?.status || "sin status";
            const errorData = jiraError?.response?.data || "sin datos";
            console.warn(`[Publish] Error al conectar con Jira (Status: ${errorStatus}, Mensaje: ${errorMsg})`);
...
exports.publishAnalysisToJira = publishAnalysisToJira;
/**
 ...
 */
const buildJiraADFComment = (analysis) => {
    ...
    const makePanelSection = (panelType, title, items) => {
        if (items.length === 0)
            return null;
        return {
            type: "panel",
 ...
 */
const commentOnJira = async (req, res) => {
    try {
        ...
        console.log(`[Comment] ADF sections: ${JSON.stringify(commentBody).length} bytes, ${commentBody.content?.length} secciones`);
        const response = await axios_1.default.post(commentUrl, { body: commentBody }, {
...
exports.analyzeAndPublish = analyzeAndPublish;
exports.default = {
    analyzeRequirement: exports.analyzeRequirement,
    publishAnalysisToJira: exports.publishAnalysisToJira,
    analyzeAndPublish: exports.analyzeAndPublish,
    commentOnJira: exports.commentOnJira,
};
//# sourceMappingURL=requirementAnalysis.controller.js.map