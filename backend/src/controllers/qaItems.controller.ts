...
type Board = {
  ...
  in_testing_age?: string | Date | null;
  sprint_id?: number | null;
  sprint_prev?: boolean | null;
  returns?: number | null;
...
export async function checkDuplicateTask(req: any, res: any) {
  await ensureTable();
  ...
  try {
    ...
    const existingTask = (rows as any[])[0];
...
export async function createQAItem(req: any, res: any) {
  await ensureTable();
  const {
    name,
    test_cases = null,
...
export async function listQAItems(req: any, res: any) {
  await ensureTable();
...
export async function getQAItem(req: any, res: any) {
  await ensureTable();
  const { id } = req.params;
  const ownerId = req.user.id;
...
export async function updateQAItem(req: any, res: any) {
  await ensureTable();
  const { id } = req.params;
  ...
  if (fields.state && fields.state === 'En pruebas') {
    ...
    const [prevRows] = await pool.query('SELECT state, in_testing_age FROM boards WHERE id = ?', [id]);
    const prevState = (prevRows as any[])[0]?.state;
    const prevTestingAge = (prevRows as any[])[0]?.in_testing_age;
  ...
  const [result] = await pool.query(
    `UPDATE boards SET ${setClause} WHERE id = ?`, 
    [...values, id]
  );
  ...
  if (affectedRows === 0) {
    ...
    if ((checkRows as any[]).length === 0) {
      ...
      const board = (checkRows as any[])[0];
...
export async function deleteQAItem(req: any, res: any) {
  await ensureTable();
  const { id } = req.params;
  const ownerId = req.user.id;
...
export async function createSprint(req: any, res: any) {
  ...
  const [result] = await pool.query(
    `INSERT INTO sprints (name, start_date, finish_date) VALUES (?, ?, ?)`,
    [
      name,
...
export async function getSprint(req: any, res: any) {
  await ensureTable();
  const { id } = req.params;
  ...
  const sprint = (rows as any)[0];
  if (!sprint) return res.status(404).json({ error: "Sprint not found" });
  res.json(sprint);
}

export async function updateSprint(req: any, res: any) {
  await ensureTable();
  const { id } = req.params;
  const fields: Record<string, any> = { ...req.body };
  ...
  const [result] = await pool.query(
    `UPDATE sprints SET ${setClause} WHERE id = ?`, 
    [...values, id]
  );
...
export async function deleteSprint(req: any, res: any) {
  await ensureTable();
  const { id } = req.params;
  
  // Primero actualizar boards para quitar la referencia al sprint
  await pool.query("UPDATE boards SET sprint_id = NULL WHERE sprint_id = ?", [id]);
...
function normalizeBoards(val: any): any[] {
  if (val == null) return [];
  if (Array.isArray(val)) return val;
  if (Buffer.isBuffer(val)) {
    try { return JSON.parse(val.toString("utf8")) ?? []; } catch { return []; }
  }
...
export async function getBoardsBySprint(req: any, res: any) {
  await ensureTable();
  ...
  try {
    ...
    const processedRows = (rows as any[]).map(r => {
      const boards = normalizeBoards(r.boards).filter((b: any) => b && b.id != null);
      return { ...r, boards };
    });
...
export async function getQAStatistics(req: any, res: any) {
  await ensureTable();
  
  try {
    // Obtener estadísticas por QA y mes basadas en fechas de prueba reales
    const [qaStatsRows] = await pool.query(`
      SELECT 
        u.name as qa_name,
        u.id as qa_id,
        DATE_FORMAT(
    ...
    const [qaGeneralRows] = await pool.query(`
      SELECT 
        u.name as qa_name,
        u.id as qa_id,
    ...
    const [devolucionesRows] = await pool.query(`
      SELECT 
        u.name as qa_name,
        u.id as qa_id,
        DATE_FORMAT(
